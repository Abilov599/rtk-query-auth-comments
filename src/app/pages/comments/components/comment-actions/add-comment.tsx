...
import { useUsePostCommentMutation } from '../../../../services/comment';
...
function AddComment() {
  const [post, { isLoading }] = useUsePostCommentMutation();
  const { user } = useAuth();
  ...
  return (
    <Form form={form} name="add-comment" onFinish={onFinish} onFinishFailed={onFinishFailed}>
      <Flex gap={10} className="max-w-[800px] mx-auto" align="center">
        ...
        <Button shape="circle" type="primary" htmlType="submit" disabled={isLoading}>
          <SendOutlined />
        </Button>
      </Flex>
    </Form>
